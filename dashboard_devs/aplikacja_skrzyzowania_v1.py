"""
Pierwsza wersja aplikacji okienkowej do przeglądania danych o skrzyżowaniach.
...
Uruchomienie:
    python aplikacja_skrzyzowania_v1.py
"""
...
def normalize_text(value: str) -> str:
    return str(value).strip().lower().replace("ą", "a").replace("ć", "c").replace("ę", "e").replace("ł", "l").replace("ń", "n").replace("ó", "o").replace("ś", "s").replace("ż", "z").replace("ź", "z")
...
class ScrollableFrame(ttk.Frame):
    ...
    def _on_mousewheel(self, event):
        # Działa na Windows/macOS; na Linuxie przewijanie nadal działa suwakiem.
        try:
...
class DataStore:
    ...
    def _load_or_interpretations(self) -> pd.DataFrame:
        try:
            path = self._find_file("OR_interpretacje.csv")
        except FileNotFoundError:
            return pd.DataFrame(columns=["Cecha", "Opis ekspercki", "Kategoria"])
        return pd.read_csv(path, sep=",", encoding="utf-8-sig")

    def _build_intersections(self) -> pd.DataFrame:
        # Główna lista pochodzi z pliku natężenia, bo zawiera po jednym wierszu na skrzyżowanie.
    ...
    def get_events(self, intersection_id: int) -> pd.DataFrame:
        return self.events[self.events["id_skrzyzowania"] == intersection_id].copy()
    ...
    def get_summary(self, intersection_id: int) -> dict:
        ...
        years = events["rok"].dropna().astype(int)
...
class IntersectionPlots:
    ...
    def event_types(self, intersection_id: int) -> Figure:
        ...
        if df.empty:
            return self._empty_figure()
        counts = df["rodzaj_zdarzenia"].value_counts().head(8).sort_values()
    ...
    def violations(self, intersection_id: int) -> Figure:
        ...
        if df.empty:
            return self._empty_figure()
        counts = df["wykroczenie"].value_counts().head(8).sort_values()
        fig, ax = plt.subplots(figsize=(6, 4.2))
        ax.barh(counts.index, counts.values, color=COLORS["primary"])
        ax.set_title("Najczęstsze wykroczenia", fontweight="bold")
        ax.set_xlabel("Liczba zdarzeń")
        ax.grid(axis="x", alpha=0.25)
        fig.tight_layout()
        return fig
    ...
    def signalling(self, intersection_id: int) -> Figure:
        ...
        if df.empty:
            return self._empty_figure()
        counts = df["sygnalizacja"].value_counts().sort_values()
        fig, ax = plt.subplots(figsize=(6, 3.5))
        ax.barh(counts.index, counts.values, color=COLORS["primary"])
        ax.set_title("Zdarzenia a sygnalizacja", fontweight="bold")
        ax.set_xlabel("Liczba zdarzeń")
        ax.grid(axis="x", alpha=0.25)
        fig.tight_layout()
        return fig
    ...
    def _count_plot(self, intersection_id: int, column: str, title: str) -> Figure:
        df = self.data.get_events(intersection_id)
        if df.empty or column not in df.columns:
            return self._empty_figure()
        counts = df[column].value_counts().head(8).sort_values()
        fig, ax = plt.subplots(figsize=(6, 3.8))
        ax.barh(counts.index, counts.values, color=COLORS["primary"])
...
class IntersectionApp(tk.Tk):
    ...
    def _build_layout(self):
        ...
        self.intersections_frame = ttk.Frame(self.notebook)
        self.stats_frame = ttk.Frame(self.notebook)

        self.notebook.add(self.home_frame, text="Home")
        self.notebook.add(self.intersections_frame, text="Skrzyżowania")
        self.notebook.add(self.stats_frame, text="Statystyki")

        self._build_placeholder(self.home_frame, "Home", "Sekcja startowa zostanie uzupełniona w kolejnej wersji aplikacji.")
        self._build_intersections_tab()
        self._build_placeholder(self.stats_frame, "Statystyki", "Sekcja statystyk i opisów wykresów zostanie uzupełniona w kolejnej wersji aplikacji.")

    def _build_placeholder(self, parent, title: str, subtitle: str):
        card = tk.Frame(parent, bg=COLORS["panel"], bd=0, highlightthickness=1, highlightbackground=COLORS["border"])
    ...
    def _build_intersections_tab(self):
        paned = ttk.PanedWindow(self.intersections_frame, orient="horizontal")
        paned.pack(fill="both", expand=True)

        # Lewy panel: lista skrzyżowań
        ...
        # Prawy panel: szczegóły skrzyżowania
    ...
    def _on_intersection_selected(self, _event=None):
        ...
        self._render_intersection(intersection_id)
    ...
    def _render_intersection(self, intersection_id: int):
        self._clear_details()
        ...
        # Dwie kolumny: informacje i wykresy
        ...
        self._render_chart_column(chart_col, intersection_id)
    ...
    def _render_info_column(self, parent, intersection_id: int):
        ...
        if attrs is not None:
            basic_rows = [
                ...
                ("Ścieżka rowerowa", fmt_value(attrs.get("ścieżka rowerowa"))),
                ("Tramwaj", fmt_value(attrs.get("tramwaj"))),
                ("Rondo", fmt_value(attrs.get("rondo"))),
            ]
        self._info_card(parent, "Dane podstawowe", basic_rows or [("Status", "Brak danych atrybutowych")])
    ...
    def _render_chart_column(self, parent, intersection_id: int):
        ...
        combo.bind("<<ComboboxSelected>>", lambda _e: self._update_charts(chart_area, intersection_id))
        ...
        self._update_charts(chart_area, intersection_id)

    def _update_charts(self, chart_area: tk.Frame, intersection_id: int):
        # Usuń wykresy z poprzedniej kategorii.
        ...
        for i, (_title, func) in enumerate(plot_specs):
...
def main():
    ...
    try:
        ...
        # Jeśli nie da się nawet utworzyć okna głównego z pełną aplikacją, pokaż prosty błąd.
        root = tk.Tk()
        root.withdraw()
